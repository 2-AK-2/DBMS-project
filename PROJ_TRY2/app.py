# --- app.py (V6 - Final Unified with Edit + Update + Search) ---
...
def search_page():
    ...
    if search_term or search_tag or search_date:
        ...
        query = base_query + (" WHERE " + " AND ".join(query_parts) if query_parts else "")
...
def update_memory_action(memory_id):
    ...
    try:
        ...
        cursor.execute("""
            UPDATE memories
            SET title = %s, description = %s, memory_date = %s
            WHERE memory_id = %s
        """, (title, description, memory_date, memory_id))
